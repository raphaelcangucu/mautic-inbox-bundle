 ...
 public function process(int $id): void {
  ...
  try{
   ...
   try{$this->em->refresh($state);$record=$this->store->find('assignment',(string)$id);$this->em->refresh($record);$fresh=$record->getData();
    if($fresh['nonce']!==$a['nonce']||$fresh['status']!=='active'||$state->getAssignee()||!$this->service->allowed($state,$agent)||$this->availability->reason($state)){ $this->store->put('run',$runKey,['status'=>'cancelled','state'=>$id]);return;}
