(function () {
    ...
    function createInboxHistory(root, onOpen, onClear) {
        var baseUrl = new URL(root.dataset.indexUrl, window.location.origin);
        var conversationTemplate = new URL(root.dataset.conversationUrl, window.location.origin);
        var escapedTemplate = conversationTemplate.pathname.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        var conversationPattern = new RegExp('^' + escapedTemplate.replace(/0\/?$/, '(\\d+)/?') + '$');
        var normalize = function (path) { return path.length > 1 ? path.replace(/\/+$/, '') : path; };
        var basePath = normalize(baseUrl.pathname);

        function current() {
            var path = normalize(window.location.pathname);
            if (path === basePath) return 0;
            var match = conversationPattern.exec(path);
            return match ? Number(match[1]) : null;
        }

        function write(stateId, replace) {
            var target = stateId ? conversationTemplate.pathname.replace(/0\/?$/, String(stateId)) : baseUrl.pathname;
            if (normalize(window.location.pathname) === normalize(target)) return;
            window.history[replace ? 'replaceState' : 'pushState']({mauticInbox: true, stateId: stateId || null}, document.title, target);
        }

        function pop(event) {
            var stateId = current();
            if (null === stateId) return;
            // Mautic's global listener reloads every popstate. Inbox entries are
            // handled locally so Back/Forward keep the workspace mounted.
            event.stopImmediatePropagation();
            if (stateId > 0) onOpen(stateId);
            else onClear();
        }
        ...
        return {
            current: current,
            open: function (stateId, replace) { write(Number(stateId), !!replace); },
            clear: function (replace) { write(0, !!replace); },
            dispose: function () { window.removeEventListener('popstate', pop, true); }
        };
    }
    ...
    function boot() {
        ...
        var inboxHistory = createInboxHistory(root, function (stateId) { select(stateId, false); }, function () { clearSelection(false); });
        ...
        function clearSelection(updateHistory) {
            ...
            $('inbox-selected').classList.add('hide');
            $('inbox-empty').classList.remove('hide');
            $('inbox-timeline').textContent = '';
            renderList();
            if (false !== updateHistory) inboxHistory.clear(false);
        }

        async function select(id, updateHistory) {
            ...
            try {
                ...
                if (false !== updateHistory) inboxHistory.open(id, false);
                state.older = null;
        ...
        var aiSelectedId=null, aiRequest=0, aiRetryBusy=null;
        ...
        function renderAiPending(reply, canAssign) {
            var card=$('inbox-ai-pending');
            if(!reply){card.className='inbox-ai-pending hide';card.removeAttribute('data-run-key');$('inbox-ai-pending-body').textContent='';return;}
            card.className='inbox-ai-pending '+(['queued','retrying','processing','blocked','failed','uncertain'].includes(reply.status)?reply.status:'queued');
            card.dataset.runKey=reply.run_key;
            $('inbox-ai-pending-title').textContent=t('mautic.inbox.ai.pending_title');
            $('inbox-ai-pending-agent').textContent=reply.agent||'';
            $('inbox-ai-pending-status').textContent=reply.status_label||reply.status;
            var body=$('inbox-ai-pending-body');body.textContent='';body.appendChild(messageBody(reply.text,state.selected&&state.selected.channel==='whatsapp'));
            var reason=reply.reason_label||'';
            if(reply.error&&reply.reason!=='new_message_received')reason+=(reason?' · ':'')+reply.error;
            $('inbox-ai-pending-reason').textContent=reason;
            var send=$('inbox-ai-send-now'),busy=aiRetryBusy===reply.run_key;
            send.textContent=busy?t('mautic.inbox.ai.sending_now'):t('mautic.inbox.ai.send_now');send.disabled=busy||!canAssign||!reply.retryable;
            $('inbox-ai-regenerate').textContent=t('mautic.inbox.ai.regenerate');$('inbox-ai-regenerate').disabled=busy||!canAssign;
        }
        async function renderAi(forceOptions) {
            ...
            if(changed){select.disabled=true;$('inbox-ai-status').textContent='';renderAiAssignment(null);renderAiPending(null);}
            try{var info=await api(endpoint('aiUrl',id));if(request!==aiRequest||!state.selected||state.selected.id!==id)return;
                ...
                aiSelectedId=id;if(state.selected&&state.selected.id===id)state.selected.version=info.version;var allowed=info.agents.filter(function(agent){return agent.allowed;});select.disabled=!info.can_assign||!allowed.length;
                var assignment=info.assignment;renderAiAssignment(assignment);
                renderAiPending(info.pending_reply,info.can_assign);
        ...
        async function resetAi(button){if(!state.selected)return;var id=state.selected.id;button.disabled=true;try{await api(endpoint('aiUrl',id),{method:'POST',body:JSON.stringify({action:'reset',version:state.selected.version})});aiSelectedId=null;await select(id);showThreadFeedback(t('mautic.inbox.ai.reset_success'));}catch(e){showThreadError(e.message,'ai');}finally{button.disabled=false;}}
        $('inbox-ai-reset').addEventListener('click',function(){resetAi(this);});
        $('inbox-ai-regenerate').addEventListener('click',function(){resetAi(this);});
        $('inbox-ai-send-now').addEventListener('click',async function(){
            if(!state.selected)return;var card=$('inbox-ai-pending'),runKey=card.dataset.runKey;if(!runKey||aiRetryBusy)return;
            var id=state.selected.id;aiRetryBusy=runKey;this.disabled=true;this.textContent=t('mautic.inbox.ai.sending_now');
            try{await api(endpoint('aiRetryUrl',id),{method:'POST',body:JSON.stringify({run_key:runKey,version:state.selected.version})});aiRetryBusy=null;aiSelectedId=null;await select(id);showThreadFeedback(t('mautic.inbox.ai.sent_now'));await loadList(false);}
            catch(e){aiRetryBusy=null;showThreadError(e.message,'ai');renderAi(true);}
        });
        ...
        root.querySelectorAll('.inbox-tabs button').forEach(function(button){button.addEventListener('click',function(){root.querySelectorAll('.inbox-tabs button').forEach(function(b){b.classList.remove('active');});button.classList.add('active');var view=button.dataset.view, staticView=view==='automation'||view==='settings';root.querySelectorAll('[data-inbox-only]').forEach(function(el){el.classList.toggle('hide',staticView);});$('inbox-automation').classList.toggle('hide',view!=='automation');$('inbox-settings').classList.toggle('hide',view!=='settings');if(!staticView){state.view=view;clearSelection(true);$('inbox-list-heading').textContent=state.view==='comments'?t("mautic.inbox.ui.comments_6fe305"):t("mautic.inbox.ui.conversations_86d0e6");loadList(false);}else if(state.selected){inboxHistory.clear(true);}});});
        ...
        $('inbox-back').addEventListener('click',function(){clearSelection(true);});
        ...
        loadList(false);
        var initialStateId = Number(root.dataset.initialStateId || inboxHistory.current() || 0);
        if (initialStateId > 0) select(initialStateId, false);
        poll();
        ...
        var cleanup = new MutationObserver(function(){if(!root.isConnected){alerts.dispose();inboxHistory.dispose();if(stream)stream.close();clearTimeout(reconnectTimer);clearInterval(fallbackTimer);clearInterval(aiActivityTimer);clearInterval(conversationActivityTimer);cleanup.disconnect();}});
