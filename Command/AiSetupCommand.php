...
final class AiSetupCommand extends Command {
 ...
 protected function execute(InputInterface $i,OutputInterface $o): int {
  ...
  if(!$this->store->find('agent','macro'))$this->store->put('agent','macro',['name'=>'Agente Macro','profile'=>'macro-support','enabled'=>true,'limit'=>8,'documents'=>['analises-esportivas','plataforma'],'permissions'=>[]]);
  if(!$this->store->find('config','global'))$this->store->put('config','global',$this->store->config());$o->writeln('AI storage ready. Explicit assignment required.');return 0;
 }
}
